...
def editSubjectList():
    try:
        ...
        section = input('Toggle which classes?: ').split(' ')
        section = [int(choice)-1 for choice in section]
        for index, className in enumerate(subjectListDB[subject]['classes'].keys()):
            if index in section:
                if className in subjectListDB[subject]['reg_classes']: subjectListDB[subject]['reg_classes'].remove(className)
                else: subjectListDB[subject]['reg_classes'].add(className)
...
def parseClasses(subject, cookie):
        ...
        return dict(zip(classNames, classIDs))

def userInClass(student_id, class_id, subject, cookie): #returns boolean
    url = mmlsClassListURL + ':' + subject['subject_id'] + ':' + subject['coordinator_id'] + ':'
    req = request.Request(url+class_id, data=None, headers={'Cookie': cookie}, method='GET')
    response = request.urlopen(req)
    tree = etree.parse(response, etree.HTMLParser())
    if tree.xpath("//table/tbody/tr/td[text()='{}']".format(student_id)):
        return True
    else:
        return False
...
def main():
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        if askYesNo('\nAutomatically fill registered classes?'):
            startTime = time.time()
            futures = [{className: executor.submit(userInClass, userid, classID, subject, cookie) for className, classID in subject['classes'].items()} for subject in subjectListDB]
